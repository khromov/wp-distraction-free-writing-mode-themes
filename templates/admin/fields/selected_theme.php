...
foreach ( glob( $dir ) as $file ) {
	if ( filetype( $file ) == 'dir' ) {
		?>
		<input type="radio" name="dfwmt_selected_theme" value="<?php echo basename( $file ) ?>"<?php echo get_option( 'dfwmt_selected_theme' ) == basename( $file ) ? ' checked' : '' ?>/> <?php echo ucfirst( basename( $file ) ) ?> -
		<a href="<?php echo $v['plugin_url'] . '/css/' . basename( $file ) . '/preview.png' ?>" target="_blank" onclick="return df_popup(this.href)"><?php _e( 'Preview', 'dfwmdt' ); ?></a>
		<br />
	<?php
	}
}
